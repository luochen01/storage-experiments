...
drop dataverse twitter_validation_norepair_$v if exists;
create dataverse twitter_validation_norepair_$v if not exists;
use  twitter_validation_norepair_$v;
...
with {
  "merge-policy": {
    "name": "prefix",
    "parameters": { "max-mergable-component-size": $merge, "max-tolerance-component-count": 5}
  }
};
