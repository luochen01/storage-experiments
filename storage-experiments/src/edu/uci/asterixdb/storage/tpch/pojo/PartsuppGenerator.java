 ...
 */
package edu.uci.asterixdb.storage.tpch.pojo;

import static com.google.common.base.Preconditions.*;
import static edu.uci.asterixdb.storage.tpch.pojo.GenerateUtils.*;
...
public class PartsuppGenerator implements TpchGenerator<Partsupp> {
    ...
    public PartsuppGenerator(double scaleFactor, int part, int partCount) {
        this(scaleFactor, part, partCount, TextPool.getDefaultTestPool());
    }

    public PartsuppGenerator(double scaleFactor, int part, int partCount, TextPool textPool) {
    ...
    @Override
    public Iterator<Partsupp> iterator() {
        return new PartSupplierGeneratorIterator(textPool, scaleFactor,
    ...
    public long cardinality() {
        return (long) (PartGenerator.SCALE_BASE * scaleFactor);
    }

    private static class PartSupplierGeneratorIterator extends AbstractIterator<Partsupp> {
        private final double scaleFactor;
        ...
        private PartSupplierGeneratorIterator(TextPool textPool, double scaleFactor, long startIndex, long rowCount) {
            ...
            availableQuantityRandom = new RandomBoundedInt(1671059989, AVAILABLE_QUANTITY_MIN, AVAILABLE_QUANTITY_MAX,
                    SUPPLIERS_PER_PART);
        ...
        @Override
        protected Partsupp computeNext() {
            if (index >= rowCount) {
                return endOfData();
            }

            Partsupp partSupplier = makePartSupplier(startIndex + index + 1);
            partSupplierNumber++;
        ...
        private Partsupp makePartSupplier(long partKey) {
            return new Partsupp(partKey, partKey, selectPartSupplier(partKey, partSupplierNumber, scaleFactor),
                    availableQuantityRandom.nextValue(), supplyCostRandom.nextValue(), commentRandom.nextValue());
        }
    }
