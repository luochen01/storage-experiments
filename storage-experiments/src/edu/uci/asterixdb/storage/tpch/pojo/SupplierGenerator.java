...
public class SupplierGenerator implements TpchGenerator<Supplier> {
    ...
    public Iterator<Supplier> iterator() {
        return new SupplierGeneratorIterator(distributions, textPool,
    ...
    private static class SupplierGeneratorIterator extends AbstractIterator<Supplier> {
        ...
        private Supplier makeSupplier(long supplierKey) {
            ...
            if (bbbCommentRandomValue <= BBB_COMMENTS_PER_SCALE_BASE) {
                ...
                buffer.replace(BBB_BASE_TEXT.length() + offset + noise,
                        BBB_BASE_TEXT.length() + offset + noise + type.length(), type);
            ...
            return new Supplier(supplierKey, supplierKey, String.format(ENGLISH, "Supplier#%09d", supplierKey),
                    addressRandom.nextValue(), nationKey, phoneRandom.nextValue(nationKey),
                    accountBalanceRandom.nextValue(), comment);
        }
    }
}
