...
seq_prefix = "twitter_validation_norepair_seq_UNIFORM_1"
...
scan_results = parse_query_experiment(prefix, scan_pattern, skips=0)
seq_scan_results = parse_query_experiment(seq_prefix, scan_pattern, skips=0)
...
def plot_bar(xvalues, options,
    ...
    if len(line_options) > 0:
        plt.text(1, random_scans[index], 'full scan')
        plt.text(0, seq_scans[index], 'full scan (sequential keys)')
...
def plot_shared_query(xvalues_1, xvalues_2, options_1, options_2, line_options, output, titles, xlabel='Query Selectivity (%)', ylabel='Query Time (s)', framealpha=0):
    ...
    plot_options(xvalues_1, options_1, ax1, titles[0], xlabel, ylimit=0)
    plot_options(xvalues_2, options_2, ax2, titles[1], xlabel, ylimit=0)
    ...
    ax2.legend(loc=2, ncol=1, framealpha=0, bbox_to_anchor=(-0.02, 1.03), handlelength=hlength, fontsize=fontsize, labelspacing=lspacing)
    ...
    ax2.text(0.1, random_scans[index], 'full scan')
    ax2.text(0.1, seq_scans[index], 'full scan (sequential keys)', bbox=box)
...
plot_shared_query(first_half(sels), second_half(sels), low_options, high_options,
               [ PlotOption(toTime(second_half(scan_results)), 'scan', marker=markers[0], linestyle='solid', color=antimatter_color),
                ...
                result_base_path + devices[index] + '-query-opt-breakdown.pdf', ["Low Query Selectivity", 'High Query Selectivity'])
