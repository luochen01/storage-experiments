...
public class DualSelectPartitionedLevelMergeScheduler extends AbstractPartitionedLevelMergeScheduler {
    private static final Logger LOGGER = LogManager.getLogger(DualSelectPartitionedLevelMergeScheduler.class);

    private final double level1Capacity;

    private MergeOperation dualMergeOp;

    public DualSelectPartitionedLevelMergeScheduler(int toleratedComponentsPerLevel, int numLevels,
    ...
    protected MergeUnit createMergeUnit(int level) {
        if (level == 0) {
            ...
            return new DualMergeUnit(level, maxNumComponentsLevel1, sizeRatioLevel0 + 1,
    ...
    public void initialize() {
        super.initialize();
        dualMergeOp = createMergeOperaiton(1, mergeSpeeds[1], new SubOperation(0, this));
    }
    ...
    public boolean isMergeable(MergeUnit mergeUnit) {
        ...
        if (mergeUnit.level == 0) {
            ...
            return ((DualMergeUnit) mergeUnit).numDualComponents > 0;
        } else {
        ...
        while (isMergeable(mergeUnit)) {
            if (mergeUnit.level + 1 < numLevels) {
                ...
                if (mergeUnit.level == 1) {
                    components = ((DualMergeUnit) mergeUnit).dualComponents;
                    numComponents = ((DualMergeUnit) mergeUnit).numDualComponents;
                } else {
                    components = mergeUnit.components;
                    numComponents = mergeUnit.numComponents;
                }

                // select the component
                if (!selectMergeComponents(mergeOp, components, numComponents, nextUnit, true)) {
                    if (DEBUG) {
                ...
                if (mergeOp.numComponentsInNextLevel == 0) {
                    ...
                    if (nextUnit.numComponents < nextUnit.maxNumComponents + nextUnit.extraNumComponents) {
                        ...
                        int remainingNumComponents =
                                removeComponent(components, numComponents, mergeOp.componentsInCurrentLevel[0]);

                        if (mergeUnit.level == 1) {
                            ((DualMergeUnit) mergeUnit).numDualComponents = remainingNumComponents;
                        } else {
                            mergeUnit.numComponents = remainingNumComponents;
                        }
                        ...
                        if (mergeUnit.level == 1 && ((DualMergeUnit) mergeUnit).readyToSwitch()) {
                            selectDualComponent(mergeUnit);
                        }
                ...
                while (mergeUnit.numComponents >= mergeUnit.maxNumComponents) {
                    if (!mergeUnit.components[mergeUnit.mergeIndex].isMerging) {
                        ...
                        mergeUnit.numComponents = removeComponent(mergeUnit.components, mergeUnit.numComponents,
                                mergeUnit.components[mergeUnit.mergeIndex]);
    ...
    private void finalizeMergeOperationInLevel0(MergeUnit mergeUnit) {
        ...
        DualMergeUnit dualUnit = (DualMergeUnit) nextUnit;
        if (dualUnit.readyToSwitch()) {
            selectDualComponent(dualUnit);
        }
        if (isMergeable(nextUnit)) {
            scheduleMergeOperation(nextUnit);
        }
    ...
    private void finalizeMergeOperation(MergeUnit mergeUnit) {
        ...
        assert sanityCheck(nextUnit.components, nextUnit.numComponents);
        if (mergeUnit.level == 1) {
            DualMergeUnit dualUnit = (DualMergeUnit) mergeUnit;
            dualUnit.numDualComponents = removeComponent(dualUnit.dualComponents, dualUnit.numDualComponents,
                    op.componentsInCurrentLevel[0]);
            assert sanityCheck(dualUnit.dualComponents, dualUnit.numDualComponents);
            if (dualUnit.readyToSwitch()) {
                // switch
                selectDualComponent(dualUnit);
                notifyBlockedUnit(mergeUnit);
            ...
            mergeUnit.numComponents =
                    removeComponent(mergeUnit.components, mergeUnit.numComponents, op.componentsInCurrentLevel[0]);
            assert sanityCheck(mergeUnit.components, mergeUnit.numComponents);
            notifyBlockedUnit(mergeUnit);
        }
    ...
    private void selectDualComponent(MergeUnit mergeUnit) {
        DualMergeUnit dualUnit = (DualMergeUnit) mergeUnit;
        assert dualUnit.numDualComponents == 0;
        assert dualUnit.numComponents > dualUnit.maxNumComponents;

        MergeUnit nextUnit = mergeUnits[dualUnit.level + 1];
        while (dualUnit.numComponents > dualUnit.maxNumComponents) {
            boolean selected =
                    selectMergeComponents(dualMergeOp, dualUnit.components, dualUnit.numComponents, nextUnit, false);
            if (!selected) {
                break;
            }
            assert dualMergeOp.numComponentsInCurrentLevel == 1;
            dualUnit.numDualComponents = addComponent(dualUnit.dualComponents, dualUnit.numDualComponents,
                    dualMergeOp.componentsInCurrentLevel[0]);
            assert sanityCheck(dualUnit.dualComponents, dualUnit.numDualComponents);

            dualMergeOp.componentsInCurrentLevel[0].pin();
            removeComponent(dualUnit, dualMergeOp.componentsInCurrentLevel[0]);
            assert sanityCheck(dualUnit.components, dualUnit.numComponents);
        }
        if (DEBUG) {
            if (dualUnit.numComponents > dualUnit.maxNumComponents) {
                LOGGER.error("Cannot select enough dual components for level 1, selected: {}, expected: {}",
                        dualUnit.numDualComponents,
                        dualUnit.numDualComponents + dualUnit.numComponents - dualUnit.maxNumComponents);
            } else {
                LOGGER.error("Select {} dual components for level 1", dualUnit.numDualComponents);
            }
