...
public class DualPartitionedLevelMergeScheduler extends AbstractPartitionedLevelMergeScheduler {
    private static final Logger LOGGER = LogManager.getLogger(DualPartitionedLevelMergeScheduler.class);

    private final double level1Capacity;

    public DualPartitionedLevelMergeScheduler(int toleratedComponentsPerLevel, int numLevels,
    ...
    protected MergeOperation createMergeOperaiton(int level, ISpeedProvider speedProvider, SubOperation subOperation) {
        if (level == 0) {
            MergeOperation op = new MergeOperation(level, 0, subOperation, this, speedProvider, sizeRatioLevel0,
                    mergeUnits[1].maxNumComponents, sizeRatioLevel0 + mergeUnits[1].maxNumComponents);
                ...
                if (mergeOp.numComponentsInNextLevel == 0) {
                    ...
                    if (nextUnit.numComponents < nextUnit.maxNumComponents + nextUnit.extraNumComponents) {
                        ...
                        if (mergeUnit.level == 1 && ((DualMergeUnit) mergeUnit).readyToSwitch()) {
                            ((DualMergeUnit) mergeUnit).switchComponents();
                        }
    ...
    private void finalizeMergeOperationInLevel0(MergeUnit mergeUnit) {
        ...
        if (dualUnit.numComponents > dualUnit.maxNumComponents && dualUnit.numDualComponents == 0) {
            dualUnit.switchComponents();
        }
        if (isMergeable(nextUnit)) {
            scheduleMergeOperation(nextUnit);
        }
